...
import { InsertStatusInput, UpdateStatusInput } from "../types/status";
...
export const statusRouter = {
	all: protectedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			if (!project || project.organizationId !== context.workspace.id) {
	...
	byId: protectedProcedure
		...
		.handler(async ({ input, context }) => {
			...
			if (!status || status.project.organizationId !== context.workspace.id) {
				throw new ORPCError("NOT_FOUND", {
